...
function TransactionEdge(props) {
  ...
  // Custom curvature based on whether it's a multi-edge
  const curvature = data?.offsetIndex > 1 ? 0.7 : 0.25;
  const [path, x, y] = getBezierPath({ sourceX, sourceY, targetX, targetY, curvature });
  return (
    <>
        ...
        <EdgeLabelRenderer>
          <div className={`hud-edge-label ${selected ? "selected" : ""}`} style={{ transform: `translate(-50%, -50%) translate(${x}px, ${y - 30}px)` }}>
            {data.relation ? (
              ...
              <>
                <div className="hud-edge-title">TXN AMT<br/><span className="hud-edge-val">{data.amount}</span></div>
                ...
                {data.offsetIndex > 1 && (
                   <div className="mt-2 pt-2 border-t border-slate-700/50">
                     <div className="hud-edge-title" style={{color: '#00f3ff'}}>Prev Bal: {data.oldBalance}</div>
                     <div className="hud-edge-title" style={{color: '#ff1e56'}}>New Bal: {data.newBalance}</div>
                     <div className="hud-edge-title mt-1">Type: {data.accountType}</div>
                     <div className="hud-edge-title">IP: {data.ip}</div>
                   </div>
                )}
              </>
            )}
  ...
  const { nodes: initialNodes, edges: initialEdges, summaryData } = useMemo(() => {
    ...
    if (data?.hops && data.hops.length > 0) {
      data.hops.forEach((hop) => {
        ...
        const pairKey = [hop.nameOrig, hop.nameDest].sort().join('-');
        edgePairCounts[pairKey] = (edgePairCounts[pairKey] || 0) + 1;
        const offsetIndex = edgePairCounts[pairKey];
        ...
        mainEdges.push({
          ...
          data: {
            ...
            oldBalance: hop.oldbalanceOrg ? `$${hop.oldbalanceOrg.toLocaleString()}` : "",
            newBalance: hop.newbalanceOrig ? `$${hop.newbalanceOrig.toLocaleString()}` : "",
            ip: hop.ipOrig || "Unknown",
